		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				echo 
				 ...
				 "</td><td>" . "<form method='POST' style='width:auto;' action='updateInfo.php?researcherid=" . $row['Researcher_ID'] .
				 "&staffid=" . $row['Staff_ID'] . "'>"
				  . "<input type='submit' value='Edit' name='updateRA'</p>"
				  . "</form>" .
				  "</td><td>" . "<form method='POST' style='width:auto;'>" .
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				echo 
				 ...
				 "</td><td>" . "<form method='POST' style='width:auto;' action='updateRA.php?first=" . $row['RA_FirstName'] .
				 "&last=" . $row['RA_LastName'] .  "'>".
				  "<input type='hidden' id='updateQueryRequest' 'name='updateQueryRequest'>"
				  . "<input type='submit' value='Edit' name='updateInfo'</p>"
				  . "</form></td><td>" . 
				  "<form method='POST' style='width:auto;' action='profile.php'>" .
